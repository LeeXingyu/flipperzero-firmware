...
#if SUBGHZ_PROTOCOL_POCSAG_ENABLE
...
void subghz_protocol_decoder_pocsag_feed(void* context, bool level, uint32_t duration) {
    ...
    if(instance->decoder.parser_step == PocsagDecoderStepReset) {
        ...
        } else if(DURATION_DIFF(duration, pocsag2400_const.te_short) <
                  pocsag2400_const.te_delta) {
...
SubGhzProtocolStatus subghz_protocol_decoder_pocsag_deserialize(
    void* context,
    FlipperFormat* flipper_format) {
    furi_assert(context);
    SubGhzProtocolDecoderPocsag* instance = context;
